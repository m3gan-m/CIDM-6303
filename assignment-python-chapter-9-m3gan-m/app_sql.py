...
"""
Notes
movies is a list of dictionaries, which were formedly in the json file
...
The for loop loops throught each ros in the list.
Therefore, each row of diciontary values is printed.
"""
"""
How to write to a Sqlite3 database
create a conneciton to a database. If database doesn't exist, it will be created.
If you get an error 'sqlite3.OperationalError.No Such Table:MOvies.'
folow Mosh's video on how to creat a table using DB browser for SQLite
print("Writing data")
"""
db_name = "db.sqlite3"
with sqlite3.connect(db_name) as conn:
    ...
    for movie in movies:
        ...
        #conn.execute() knows to replace the question marks in the INSERT INTO
        #statement with values form tuple(movies.value())
        conn.execute(sql_command, movie_values)
    #Save the changes to the database. Notice the indentation is outside
    #the for loop by inside the with block
    conn.commit()
    ...
    2. IMPORTANT for future assignment:
    conn.execute(sql_command, values) requires a list or tuple as the values argument
    In this example, if movies was already a list of list or list of tuples,
    you would not need to convert movie to a tuple on line 53.
    Because movie is a diciontary, we have to convert movie.values() to a tuple or a list
