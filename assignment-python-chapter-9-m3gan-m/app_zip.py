...
"""
Questions to consider about this code. What is .rglob() and how is it different form .glob()?
see video 9.3. What does "w" argument mean? see video 9.5
If you only wanted .txt files, whic variable would you change to what value? "*.txt"
"""
...
if the file to unzip is not in the current directory, the source_to_unzip could also be 
the directory name and the file name combined ex "ecommerce/file.zip"
The target_directory is the director where you want the contents to be unzipped to
The target_directory can be any directory
In this example "extract" will be the name of the directory to where the files will be unzipped
"""
